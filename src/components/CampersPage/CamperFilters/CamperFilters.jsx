...
const typeOptions = [
  {
    icon: <Icon name="bi_grid-1x2" width={32} height={32} />,
    label: "Van",
    value: "van",
    class: "",
  },
  {
    icon: <Icon name="bi_grid" width={40} height={32} />,
    label: "Fully Integrated",
    value: "fully-integrated",
    class: "fullyIntegrated",
  },
  {
    icon: <Icon name="bi_grid-3x3-gap" width={40} height={32} />,
    label: "Alcove",
    value: "alcove",
    class: "",
  },
...
const filterableBadges = FEATURE_BADGES.filter(
  (b) => !["transmission", "engine"].includes(b.key)
);
  ...
  return (
    <aside className={styles.sidebar}>
      <div className={styles.filterBlock}>
        <label className={styles.filterLabel} htmlFor="location-input">
          Location
        </label>
        <div className={styles.locationBox}>
          <span className={styles.icon}>
            <Icon
              name={
                filters.location ? "location-selected" : "location-unselected"
              }
              width={20}
              height={20}
            />
            ...
            {filterableBadges.map((opt) => (
              <button
                ...
                className={`${styles.filterBtn} ${
                  filters.equipment?.includes(opt.key) ? styles.active : ""
                }`}
        ...
        <div className={styles.filterGroup}>
          ...
          <div className={styles.filterOptions}>
            {typeOptions.map((opt) => (
              <button
                key={opt.value}
                className={`${styles.filterBtn} ${styles[opt.class]} ${
                  filters.vehicleType === opt.value ? styles.active : ""
                }`}
